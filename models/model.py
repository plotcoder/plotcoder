...
class PlotCodeGenerator(nn.Module):
	def __init__(self, args, word_vocab, code_vocab):
		...
		if self.bow:
			...
			self.classifier = mlp.MLPModel(self.num_MLP_layers, self.input_size, self.MLP_hidden_size, self.num_plot_types, self.dropout_rate, self.cuda_flag)
		else:
			...
			if self.label_prediction:
				self.plot_type_classifier = mlp.MLPModel(self.num_MLP_layers, self.LSTM_hidden_size * 4, self.MLP_hidden_size, self.num_plot_types, self.dropout_rate, self.cuda_flag)
	...
	def forward(self, batch_input, batch_labels, eval_flag=False):
		...
		if self.bow:
			...
			if self.label_prediction:
				plot_type_classifier_input = torch.cat([nl_encoder_output[:,-1, :], code_encoder_output[:, -1, :]], dim=-1)
