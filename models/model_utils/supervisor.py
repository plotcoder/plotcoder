...
class Supervisor(object):
	...
	def train(self, batch_input, batch_labels):
		...
		if self.bow:
			cur_loss, pred_logits = self.model(batch_input, batch_labels)
			pred_labels = pred_logits.argmax(dim=1)
			pred_acc = torch.sum(torch.eq(pred_labels, batch_labels))
			pred_acc = pred_acc.item() * 1.0 / batch_labels.size()[0]
		else:
