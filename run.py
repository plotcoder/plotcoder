...
def evaluate(args):
	...
	for i, item in enumerate(test_data):
		...
		if args.bow:
			...
			for key in init_test_data[test_idx]:
				if key == 'context':
					continue
				print('key: ', key)
				print(init_test_data[test_idx][key])
			print('preprocessed code context: ', data_processor.ids_to_prog(item, item['input_code_seq']))
			print('preprocessed nl context: ', [data_processor.word_vocab_list[tok] for tok in item['input_word_seq']])
			print('df list: ', item['reserved_dfs'])
			print('str list: ', item['reserved_strs'])
			print('var list: ', item['reserved_vars'])
			print('target df list: ', item['target_dfs'])
			print('target str list: ', item['target_strs'])
			print('target var list: ', item['target_vars'])
			print('prediction: ', pred_prog)
